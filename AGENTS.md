...
> 중첩 디렉토리의 `AGENTS.md`(예: `glasses-app/AGENTS.md`)도 함께 읽힌다.
...
현재 활발히 개발 중인 통합 데모는 `glasses-app/` 이며, 핵심 플로우는:
...
- **`glasses-app/ProjectSettings/ProjectVersion.txt` 가 `2022.3.62f3` 임을 빌드 전에 확인** (현재 그렇게 되어 있음).
...
- 실수로 6000 으로 열어 오염됐다면: `git -C C:/claude/staging/snucapstone-ar/repo checkout -- glasses-app/ProjectSettings/` 로 되돌리고 다시 2022.3.62f3 로 연다.
...
- 진입점 = `BuildSpatialAnchorTest.PerformBuild` (`glasses-app/Assets/Editor/BuildSpatialAnchorTest.cs`). 출력 APK 경로는 같은 파일의 `OUTPUT_APK` 상수.
- 성공 시 로그에 `=== SUCCEEDED ===`, 실패 시 `=== FAILED ===` + `EditorApplication.Exit(1)`.
- PowerShell 에서는 동봉된 `glasses-app/build_2022.ps1` 도 있으나 **`PROJECT_DIR` 가 옛 경로(`C:\claude\staging\spatial_anchor_unity_2022`)를 가리킨다** — 이 레포 경로로 빌드하려면 위 명령을 직접 쓰는 게 안전하다.
...
- 둘 다 `BUILD_TAG` 와 **일치**해야 함. 불일치면 Bee 캐시 stale → `glasses-app/Library/Bee` 정리 후 재빌드.
...
adb -s $SER install -r .../glasses-app/Build/EagleEye-SA-bNN.apk
...
conquest 파이프라인 컴포넌트 (모두 `glasses-app/Assets/` 하위):
...
- **대시보드**: `glasses-app/tools/monitor/eagle_monitor.py` (순수 stdlib). `adb logcat` 파싱 → 풀스크린 렌더(CLIP 플래그 / 카운터 트리 / SLAM 센서 / 소환 객체 위치 / 이벤트 피드: 색판정·MATCH·DIVERGED). adb 끊김 자동 재시도.
- **스킬**: `/eagle-monitor` (`.claude/skills/eagle-monitor/SKILL.md`, user-invocable) → 새 터미널 창에 대시보드. 수동: `python glasses-app/tools/monitor/eagle_monitor.py` (옵션 `--adb`/`--serial`/`--demo`). **앱은 b16+ 빌드**라야 `[MONITOR]` emit.
- **안경 화면 HUD**(좌상 카운터 + CLIP 플래그)도 같은 데이터를 표시.

---

## 4d. 크래시 / SLAM 8Hz / OpenXR 표면 — 확정 진단 (2026-06-11)

★ **상세 단일출처: [`docs/findings-2026-06-11-crash-slam-openxr.md`](docs/findings-2026-06-11-crash-slam-openxr.md)** — 아래는 요약. 이 4가지는 재조사 불필요(코드/.so/로그로 확정).

- **앱 실행 시 기기 꺼짐 = 공유 Hexagon CDSP 충돌** [확정]. 우리 CLIP(QNN HTP, `AndroidManifest.xml:24` libcdsprpc.so + 매 콜드런치 125초 컴파일)이 RayNeo SLAM(FFVINS)과 같은 CDSP(domain 3)를 써서 CDSP user-PD를 터뜨림(SSR) → XR 런타임 FastRPC 핸들 stale(`remote_handle64_invoke failed ... domain 3` flood) → `system_server` 사망 → 프레임워크 재시작 → **`adb reboot`로만 복구**. "우리 앱만 / 첫 실행 OK·이후 크래시"가 결정 증거. 증폭원인=b15 `ATWSupport 0→1`.
  - **수정 = CLIP을 CPU(XNNPACK)로** (`ClipExtractor.useNpu=false`, `QnnClipEngine.initializeCpu`). 우리 CDSP 사용 0 → 구조적 차단 + 125초 컴파일 소멸. **b17에 코드 준비됨, 빌드/디바이스 검증 대기.** GPU는 INT8 미지원이라 불가(FP16 재export 필요).
- **SLAM world-anchor ~8Hz judder = 고칠 레이트 아님** [확정]. RayNeo 런타임 고정 FFVINS 갱신율(설정 노브 0건). ATW가 **회전전용**(`depthSubmissionMode:0` + 컴포지터 spacewarp 경로 없음)이라 **translation 미재투영** + 코앞 quad(parallax 최대)라 가시. 벤더 큐브 데모는 더 빠른 게 아니라 parallax 작음. 레버: parallax 완화 / gyro 회전보간(둘 다 b17 fold-in 가능) — depth제출·DOF3은 금지.
- **RayNeo OpenXR 제공/미제공 전체 지도** [확정, findings §3]. 제공: SLAM on/off·리센터·상태, 카메라(+intrinsics fx376.686/fy376.1188/cx319.3743/cy241.355)·TakePicture, ATW(회전전용), 입력(ring/시선), **평면검출(미사용)**, SendCommand/SetProp. **미제공**: VIEW-space 컴포지션 레이어(→고정HUD불가), 예측/직접 포즈 폴링(`RayNeoApi_GetHeadTrackerPose` 미export=죽음), depth/spacewarp, SLAM 컴퓨트 배치 제어, 공간앵커, IMU C# 바인딩.
  - **미사용 기회 3개**: ①intrinsics로 광고 depth 배치 ②평면검출으로 책상 표면 앵커(8Hz judder도 완화) ③시선 pose로 트리거 업그레이드.
- **통합 최소-빌드**: 크래시 수정+DSP경합+8Hz+카메라경합 측정이 **단 1빌드(b17)**로 수렴. 시퀀스: A(b16 0빌드 logcat 캡처로 인과 박제) → B(b17 reboot 후 ≥5회 launch, `useNpu` A/B + 카메라 토글 측정).
