...
> Status: **v1 PoC — 온디바이스 end-to-end 동작 확인** (6DoF SLAM 공간고정 + CLIP/OCR 매칭 + world-anchored 광고). **현황 정본은 [`docs/STATUS.md`](docs/STATUS.md)** (최신 버전·매칭 아키텍처·다음 작업), 실험 history 는 [`docs/EXPERIMENTS.md`](docs/EXPERIMENTS.md).
> 타겟 하드웨어: **RayNeo X3 Pro** (Snapdragon AR1 Gen 1, Hexagon v73 NPU)
> 표시 방식: **3D world-anchored 광고** + 2D HUD 병행
> 처음 들어왔다면: 전체 맥락·읽는 순서는 [`CLAUDE.md`](CLAUDE.md) "문서 읽는 법" → [`docs/`](docs/) 순으로 본다 (옛 ONBOARDING.md 핸드오프는 `docs/archive/` 로 동결).
...
> **안경 앱 본체는 [`glasses-app/`](glasses-app/)** (자기완결 Unity 프로젝트). `offline/` 의 `*.py` 와 `offline/db/`·`offline/products/` 는 **Mac 측 오프라인 도구/데이터**다. 전체 맥락·history 는 [`docs/`](docs/) 참조 ([CLAUDE.md](CLAUDE.md) 의 "문서 읽는 법").

```
.
├── glasses-app/                  # ★ 안경 앱 (Unity). Assets/Scripts(C#) · Assets/Plugins/Android(Java NPU)
│                                 #   · Assets/Editor(빌드 진입점) · Assets/StreamingAssets(모델+db) · *.ps1(Windows 빌드) · tools/monitor
├── docs/                         # source of truth. STATUS(현황 정본) / vision / EXPERIMENTS(실험 history) / client-spec / dev-guide / platform-rayneo / progress-log / archive(동결)
│
├── offline/                      # (Mac) 오프라인 도구/데이터 — 안경 런타임 코드 아님
│   ├── yolo.py                   #   PoC 프로토타입 파이프라인
│   ├── yolo_bbox_only.py         #   YOLO bbox-only 변형
│   ├── eagle_eye_step2.py        #   YOLO 검출 단독 실행
│   ├── create_embeddings.py      #   상품 이미지 → 임베딩 DB 생성
│   ├── eagle_eye_ad_db.py        #   광고 DB CRUD 헬퍼
│   ├── create_ad.py              #   광고 카드 이미지 생성 헬퍼
│   ├── diag_stability.py         #   Optical Flow 안정 감지 디버깅
│   ├── make_ppt.py               #   발표자료 생성 헬퍼
...
│   ├── clean_onnx_for_qnn.py     #   ONNX export 결과를 QNN 컴파일러용으로 정리
...
대용량 모델은 git에 포함되어 있지 않습니다. 아래 순서대로 받으세요. **오프라인 도구는 모두 `offline/` 안에서 실행**하므로 먼저 `cd offline` 한 뒤 아래 명령을 따른다 (경로는 `offline/` 기준).
...
자세한 분석은 [`docs/dev-guide.md`](docs/dev-guide.md) (ONNX/NPU 섹션), [`offline/qai_hub_results.json`](offline/qai_hub_results.json) 참고.
