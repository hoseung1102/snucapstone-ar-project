...
### 무선이 자꾸 끊길 때 — 지속성 높이기 (기기·공유기 쪽)

`adb tcpip 5555` 는 본질적으로 영속적이지 않고(재부팅·절전·Wi-Fi 파워세이브에 약함), **scrcpy 도 keep-alive/자동재연결을 제공하지 않는다**(공식 문서 확인 — 연결 끊기면 종료). 따라서 지속성은 **기기·공유기 설정**으로 만든다:

1. **IP 고정 — 공유기 DHCP 예약**: 공유기 관리페이지에서 안경 MAC 에 고정 IP 예약 → IP 가 절대 안 바뀜("IP 가 바뀌나?" 변수를 제거). 안경 MAC 은 `adb -s <SER> shell ip link show wlan0` 로 확인.
2. **재부팅에도 유지 — Android 12 무선 디버깅**: 안경 설정 → 개발자 옵션 → **무선 디버깅(Wireless debugging) ON**. 레거시 `adb tcpip` 과 달리 토글이 켜져 있으면 재부팅 후에도 adb-over-Wi-Fi 가 유지된다. 최초 1회 `adb pair <IP>:<pairPort>`(개발자 옵션에 뜨는 페어링 코드) → 이후 `adb connect <IP>:<port>`.
3. **절전에 안 끊기게**: 개발자 옵션 → "**Wi-Fi 를 절전 중에도 켜둠 = 항상(Always)**" + "**Stay awake**(충전 중 화면 유지)" + 디스플레이 sleep=never. 안경은 **벗으면(착용 센서) 잠들 수 있으니** 검증 중엔 착용하거나 깨워둔다.
4. **scrcpy 공식 편의(빠른 복구)**: `scrcpy --tcpip`(USB 연결 상태면 IP 자동검출+tcpip 활성+연결+미러 한 번에), `scrcpy --tcpip=+<IP>`(알려진 IP 로 **강제 재연결**). 끊겼을 때 한 줄 복구.

> **권장 조합**: ① 공유기 IP 예약 + ② 개발자 옵션 무선 디버깅 ON + ③ Wi-Fi 항상 켜둠. → 재부팅·절전에도 대체로 살아있고, 끊겨도 `scrcpy --tcpip=+<IP>` 로 즉시 복구. (🟠 무선 디버깅 토글 경로는 이 머신에서 아직 미검증 — 켜고 한 번 pair 하면 🟢 승격.)
