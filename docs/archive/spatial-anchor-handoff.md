...
- **프로젝트**: `glasses-app/` (Unity 2022.3.62f3, RayNeo X3 Pro / Snapdragon AR1 Gen1 / Hexagon v73 NPU)
...
- 만약 실수로 6000 으로 열어 오염됐다면: `git -C <repo> checkout -- glasses-app/ProjectSettings/` 로 되돌리고 다시 2022.3.62f3 로 연다.
...
adb -s $SER install -r .../glasses-app/Build/EagleEye-SA-b14.apk
...
| 파일 | 무엇 |
|---|---|
| `glasses-app/Assets/Scripts/HelloAR.cs` | 파이프라인 오케스트레이터. 모든 데모 플래그 단일 셋업. 색 brand 통합 지점(`ResolveBrandByColor`, `:329`) |
| `glasses-app/Assets/Scripts/ClipExtractor.cs` | CLIP 임베딩 + 중앙 crop 전처리 + **평균색 산출**(`:248-279`). worker `isReady` 폴링(`:160-179`) |
| `glasses-app/Assets/Plugins/Android/QnnClipEngine.java` | CLIP HTP 컴파일을 **single-thread worker 로 격리**(`:59-110`). 프리베이크 경로(`:92`). HTP 캐시 옵션(`:151-162`, 디스크 미작동) |
| `glasses-app/Assets/Scripts/SpatialAnchorTest.cs` | 6DoF SLAM, world-anchored 광고 spawn(`ShowAdBesideMatch :259`), max2 FIFO(`:297`), 가로반전(`:333`), 발산 재앵커(`:422-443`), HUD |
| `glasses-app/Assets/Scripts/CameraPreview.cs` | ShareCamera 라이프사이클, `OnApplicationPause` close(`:210`) |
